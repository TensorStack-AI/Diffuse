...
namespace Diffuse.Views
{
    ...
    /// </summary>
    public partial class ControlNetImage : ViewBaseDiffusion
    {
        private ImageInput _sourceImage;
        private ImageInput _extractImage;

        /// <summary>
        /// Initializes a new instance of the <see cref="ControlNetImage"/> class.
        /// </summary>
        public ControlNetImage(Settings settings, NavigationService navigationService, IEnvironmentService environmentService, IDownloadService downloadService, IDiffusionService diffusionService, IExtractService extractService, IUpscaleService upscaleService, IHistoryService historyService, ILogger<ControlNetImage> logger)
        ...
        public override View View => View.ControlNetImage;
        ...
        protected override async Task ExecuteAsync()
        {
            ...
            Logger.LogInformation($"[ControlNetImage] [Execute] Executing pipeline...");
            ...
            try
            {
                ...
                // Diffusion
                var options = Options with { InputControlImage = _sourceImage };
                ...
                Logger.LogInformation("[ControlNetImage] [Execute] Executing pipeline complete, Elapsed: {Elapsed:c}", Stopwatch.GetElapsedTime(timestamp));
            }
            catch (OperationCanceledException)
            {
                Statistics.Clear();
                Logger.LogInformation("[ControlNetImage] [Execute] Executing pipeline cancelled, Elapsed: {Elapsed:c}", Stopwatch.GetElapsedTime(timestamp));
            }
            catch (Exception ex)
            {
                Statistics.Clear();
                IsPipelineLoaded = DiffusionService.IsLoaded;
                Logger.LogError(ex, "[ControlNetImage] [Execute] An exception occurred executing pipeline, Elapsed: {Elapsed:c}", Stopwatch.GetElapsedTime(timestamp));
        ...
        {
            Logger.LogInformation($"[ControlNetImage] [SaveHistory] Saving history...");
            ...
            Logger.LogInformation($"[ControlNetImage] [SaveHistory] History saved.");
            return result;
        }
    }
}